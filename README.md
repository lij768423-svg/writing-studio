...
- **写作参考结构清理**：`写作经验教训.md` 从 217 行精简到 173 行（-20%），把工具链问题移到 `写作参考/tools/CHANGELOG.md`，把 orchestrator bug 移到 `scripts/CHANGELOG.md`——复盘条目只关注"写作"本身
- **新增 2 个 CHANGELOG**：
  - `写作参考/tools/CHANGELOG.md` — 工具链（tvly / mmx / draw.io / last30days / PATH 速查表）
  - `scripts/CHANGELOG.md` — orchestrator 维护（state 机 / 脆性命令 / 已弃用 / 已修 bug）
- **新归档策略**：2026-06-10+ 的新归档文章不再 commit——`归档/` 是本地工作产出历史，不进 git 跟踪。已在 `.gitignore` 加 `归档/*-2026-06-1*/` 规则（未来新归档按日期加 ignore glob）
- **新工作流规则**（用户 2026-06-10 定）：draft 写好后必须**先给用户审**（让用户审）→ OK 后才进 subagent 审校 → 应用审校 → final → 归档。用户审是必经节点不是可选项
- **新增归档选题**：「5 个 agent 之后——我为什么不再问人」（马原课堂分享）—— 4 哲学家递进逻辑（黑格尔→列维纳斯→哈贝马斯→马克思）+ 现实案例贯穿（GPT/Claude Code/Codex/Hermes/OpenClaw）+ 哲学翻译成人话（"面容"→"真正的朋友会拒绝你"）+ 阶段性答案模式（"这是我目前给自己的答案"）
...
- **2026-06-10** · 5 个 agent 之后——我为什么不再问人（马原课堂分享） ⚠️ 不进 git
- **2026-06-10** · 商品拜物教 2.0——马原课堂分享 ⚠️ 不进 git（备用）
...
> ⚠️ 2026-06-10+ 归档不进 git——`归档/` 是本地工作产出历史，不进 git 跟踪（详见 .gitignore）
